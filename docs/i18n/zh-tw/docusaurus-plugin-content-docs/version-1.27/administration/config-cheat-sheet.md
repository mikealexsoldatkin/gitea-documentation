...
slug: "config-cheat-sheet-zh-tw-v1.27"
sidebar_position: 30
...
- `ALLOW_FORK_INTO_SAME_OWNER`: **false**: 允許將儲存庫派生到同一所有者（使用者或組織）中。此功能為實驗性功能，尚未完全測試，將來可能會更改。
...
- `RETARGET_CHILDREN_ON_MERGE`: **true**: 當父合併請求被合併時，將子合併請求重新定向到父合併請求的目標分支。僅適用於頭分支和基分支指向同一儲存庫的已合併合併請求。
- `DEFAULT_DELETE_BRANCH_AFTER_MERGE`: **false**: 為新儲存庫設定"合併後預設刪除合併請求分支"的預設值。
...
- `FILE_MAX_SIZE`: **2048**: 版本發佈附件的最大文件大小限制（MB）
- `MAX_FILES`: **5**: 版本發佈一次最多上傳的附件數量。
...
- `FILE_ICON_THEME`: **material**: 文件圖標主題（basic/material）。
- `FOLDER_ICON_THEME`: **basic**: 文件夾圖標主題（basic/material）。
- `MAX_DISPLAY_FILE_SIZE`: **8388608**: 能夠顯示文件的最大大小（預設為 8MiB）。
- `AMBIGUOUS_UNICODE_DETECTION`: **true**: 檢測文件內容中的歧義 Unicode 字符並在介面上顯示警告。
...
- `REACTION_MAX_USER_NUM`: **10**: 更改在反應工具提示中顯示的使用者數量（鼠標懸停時觸發）。
- `CUSTOM_EMOJIS`: **gitea, codeberg, gitlab, git, github, gogs**: 不在 utf8 標準中定義的額外表情符號。
  ...
  並將其添加到此設定中。
- `ENABLED_EMOJIS`: **_empty_**: 以逗號分隔的啟用表情符號列表，例如："smile, thumbsup, thumbsdown"。留空以啟用所有表情符號。
...
- `EXPLORE_PAGING_DEFAULT_SORT`: **recentupdate**: 更改探索頁面的排序類型。有效值為 "recentupdate"、"alphabetically"、"reverselastlogin"、"newest" 和 "oldest"。
- `PREFERRED_TIMESTAMP_TENSE`: **mixed**: 所有時間戳應呈現的時態。可選值為 `absolute` 時間（即 1970-01-01, 11:59）和 `mixed`。`mixed` 表示大部分時間戳以相對時間顯示（即 2 天前）。
...
- `RENDER_OPTIONS_COMMENT`: **short-issue-pattern, new-line-hard-break**: 自訂不同上下文的渲染選項。
  設定為 "none" 以禁用預設值，或使用逗號分隔的列表：
  - short-issue-pattern: 識別 "#123" 工單引用並將其渲染為指向工單的鏈接
  - new-line-hard-break: 將軟換行符渲染為硬換行符，這意味著段落之間的單個換行符將導致換行，
    並且不需要在段落後添加尾隨空格來強制換行。
- `RENDER_OPTIONS_WIKI`: **short-issue-pattern**: 參見 RENDER_OPTIONS_COMMENT
- `RENDER_OPTIONS_REPO_FILE`: **_empty_**: 參見 RENDER_OPTIONS_COMMENT
...
- `MATH_CODE_BLOCK_DETECTION`: **inline-dollar,block-dollar**: 啟用數學程式碼塊檢測的分隔符。
  設定為 "none" 以禁用全部，或使用逗號分隔的列表：inline-dollar, inline-parentheses, block-dollar, block-square-brackets。
  預設值遵循 GitHub 的行為。
...
- `ROOT_URL`: **`{PROTOCOL}://{DOMAIN}:{HTTP_PORT}/`**:
  覆蓋自動生成的公共 URL。
  這個選項用於內部 URL 和外部 URL 不匹配的情況（例如 Gitea 運行在反向代理後）。
- `PUBLIC_URL_DETECTION`: **`legacy`**: 控制如何生成公共 URL。
   雖然它的預設值是 "legacy"（避免影響老的使用者），大多數實例都應當使用 "auto" 行為，尤其是 Gitea 實例需要在容器網路中被訪問的場景。
   - "legacy": 當 "X-Forwarded-Proto" 請求頭存在的時候，使用 "Host" 請求頭生成公共 URL，否則使用 "ROOT_URL"。
   - "auto": 總是使用 "Host" 請求頭生成公共 URL，如果 "X-Forwarded-Proto" 請求頭存在也會使用它。如果沒有 "Host" 請求頭則使用 "ROOT_URL"。
   - "never": 總是使用 "ROOT_URL" 生成公共 URL，不從請求頭進行檢測。
...
- `LFS_ALLOW_PURE_SSH`: **false**: 啟用 Git LFS 純 SSH 協議支援。當前預設禁用，參見 [Git LFS 支援](administration/git-lfs-support.md)。
...
- `LFS_MAX_BATCH_SIZE`: **0**: 客戶端可通過 LFS 批量 API 請求的最大 LFS 指針數量。零為預設行為，允許任意大小的批量請求。
...
- `CHARSET_COLLATION`: **_empty_**:（僅限 MySQL/MSSQL）Gitea 期望使用區分大小寫的排序規則。留空以使用 Gitea 決定的預設排序規則。除非您明確知道需要什麼，否則不要更改它。
...
- `SLOW_QUERY_THRESHOLD` **5s**: 超過此秒數閾值的查詢執行時間將在 xorm 日誌中記錄為警告。

請參見 #8540 和 #8273 以獲取有關 `MAX_OPEN_CONNS`、`MAX_IDLE_CONNS` 和 `CONN_MAX_LIFETIME` 的適當值及其與端口耗盡的關係的進一步討論。

## 索引 (`indexer`)

- `ISSUE_INDEXER_TYPE`: **bleve**: 工單索引類型，當前支援：`bleve`、`db`、`elasticsearch`（也相容 OpenSearch）或 `meilisearch`。
...
- `REPO_INDEXER_TYPE`：**bleve**: 程式碼搜索引擎類型，可以為 `bleve` 或者 `elasticsearch`（也相容 OpenSearch）。
...
- `EXTERNAL_USER_DISABLE_FEATURES`：**_empty_**：僅在使用者使用外部登入方式（例如 LDAP、OAuth 等）時禁用的功能列表，以逗號分隔，可選值為 `deletion`、`manage_ssh_keys`、`manage_gpg_keys`、`manage_mfa`、`manage_credentials`。此設定獨立於 `USER_DISABLED_FEATURES`，並作為其行為的補充。
  - `deletion`：使用者不能刪除自己的帳戶。
  - `manage_ssh_keys`：使用者不能設定 SSH 密鑰。
  - `manage_gpg_keys`：使用者不能設定 GPG 密鑰。
  - `manage_mfa`：使用者不能設定 MFA 設備。
  - `manage_credentials`：使用者不能設定郵箱、密碼或 OpenID。
  - `change_username`：使用者不能更改其使用者名稱。
  - `change_full_name`：使用者不能更改其全名。
...
- `REVERSE_PROXY_LOGOUT_REDIRECT`: **_empty_**: 當使用反向代理或 SSO 進行身份認證時，使用者登出後重定向到的 URL 或相對路徑。例如：`/my-sso/logout?return=/my-sso/home`。
...
- `DISABLE_QUERY_AUTH_TOKEN`: **false**: 拒絕通過 URL 查詢字符串發送的 API 令牌（僅接受基於 Header 的 API 令牌）。此設定將在 Gitea 1.23 中預設為 `true`，並在 Gitea 1.24 中棄用。
- `TWO_FACTOR_AUTH`：**_empty_**：設定為 enforced 以強制啟用雙因素認證。僅在 Gitea 1.24 及更高版本中可用。
...
- `ENABLE_PASSWORD_SIGNIN_FORM`：**true**：顯示密碼登入表單（用於基於密碼的登入），否則僅顯示已啟用的 OAuth2 或 Passkey 登入方式。如果設定為 false，可能還需要將 `ENABLE_BASIC_AUTHENTICATION` 設定為 false 以完全禁用基於密碼的身份驗證。
- `ENABLE_PASSKEY_AUTHENTICATION`：**true**：允許使用者使用 Passkey 登入。
- `ENABLE_REVERSE_PROXY_AUTHENTICATION`: **false**: 啟用此項以允許反向代理身份驗證。
- `ENABLE_REVERSE_PROXY_AUTHENTICATION_API`：**false**：啟用此項以允許 API 請求的反向代理身份驗證，反向代理負責確保不會發生 CSRF 攻擊。
...
- `ENABLED`: **false**: 啟用請求服務質量和過載保護。
- `MAX_INFLIGHT`: **（動態）**: 伺服器在將新請求加入隊列之前將處理的最大併發請求數。預設值為"CpuNum * 4"。
- `MAX_WAITING`: **100**: 在新請求被丟棄之前可以加入隊列的最大請求數。
- `TARGET_WAIT_TIME`: **250ms**: 請求在隊列中等待的目標最大時間。等待時間低於此值的請求不會被丟棄。當等待時間超過此值時，將丟棄部分請求，直到等待時間降低到此值以下。
...
- `FROM_DISPLAY_NAME_FORMAT`：**`{{ .DisplayName }}`**：當 Gitea 代表使用者發送郵件時，將使用 WebUI 中顯示的名稱。如果您希望例如 `Mister X (by CodeIt) <gitea@codeit.net>`，請設定為 `{{ .DisplayName }} (by {{ .AppName }})`。可用變量：`.DisplayName`、`.AppName` 和 `.Domain`。
...
- `EMBED_ATTACHMENT_IMAGES`: **false**: 在 HTML 郵件中以 base64 格式嵌入附件圖片。（適用於不加載外部圖片的客戶端或斷開 VPN 連接仍接收郵件的使用者；注意：gmail 等在線 Web 客戶端不會顯示 base64 嵌入的圖片）

## 覆蓋郵件頭（`mailer.override_header`）
...
- `STORAGE_TYPE`: **local**: 附件的儲存類型，可以為空、`local`、`minio`、`azureblob` 或在 `[storage.xxx]` 中定義的其他名稱。

  當 `STORAGE_TYPE` 為空或未設定此項時，所有儲存將從 `[storage]`（如已設定）或預設值派生。

  當 `STORAGE_TYPE = local` 時，以下為可用設定項

  - `PATH`: **attachments**: 儲存附件的路徑，僅當 STORAGE_TYPE 為 `local` 時可用。如果是相對路徑，將會被解析為 `{AppDataPath}/{attachment.PATH}`。

  當 `STORAGE_TYPE = minio` 時，相關設定詳見 `Minio 儲存設定`，您也可以如下定義設定來覆蓋派生的或預設的值。

  - `MINIO_BASE_PATH`: **attachments/**: Minio 儲存桶上的基本路徑，僅當 STORAGE_TYPE 為 `minio` 時可用。

  當 `STORAGE_TYPE = xxx` 時，設定將從 `[storage.xxx]` 派生，以下設定項可被覆蓋。
...
- `NOTICE_ON_SUCCESS`: **false**: 是否在成功執行時也發出通知。
...
- `ENABLED`: **true**: 啟用定期運行更新鏡像任務。
- `SCHEDULE`: **@every 10m**: 使用 Cron 語法的定時任務觸發設定，例如 `@every 3h`。
- `RUN_AT_START`: **false**: 在 Gitea 啟動時運行更新鏡像任務。
- `NOTICE_ON_SUCCESS`: **false**: 是否在成功時發出通知。
...
- `ENABLED`: **true**: 啟用定期運行儲存庫健康檢查任務。
- `SCHEDULE`: **@midnight**: Cron 語法，用於安排儲存庫健康檢查。
- `RUN_AT_START`: **false**: 在 Gitea 啟動時運行儲存庫健康檢查任務。
- `NOTICE_ON_SUCCESS`: **false**: 是否在成功時發出通知。
...
- `ENABLED`: **true**: 啟用更新遷移海報 ID 任務。
- `RUN_AT_START`: **true**: 在啟動伺服器時更新遷移儲存庫的工單和評論的海報 ID。
- `NOTICE_ON_SUCCESS`: **false**: 是否在成功時發出通知。
- `SCHEDULE`: **@midnight** : 同步之間的間隔作為持續時間，每次實例啟動時都會嘗試同步。

#### Cron - 同步外部使用者 (`cron.sync_external_users`)

- `ENABLED`: **true**: 啟用同步外部使用者資料任務。
- `RUN_AT_START`: **false**: 在啟動伺服器時同步外部使用者資料。
- `NOTICE_ON_SUCCESS`: **false**: 是否在成功時發出通知。
- `SCHEDULE`: **@midnight** : 同步之間的間隔作為持續時間，每次實例啟動時都會嘗試同步。
- `UPDATE_EXISTING`: **true**: 建立新使用者，更新現有使用者資料，並禁用不再在外部源中的使用者（預設設定）或僅在 UPDATE_EXISTING 設定為 false 時建立新使用者。

#### Cron - 清理過期的 Actions 資源 (`cron.cleanup_actions`)

- `ENABLED`: **true**: 啟用清理過期 Actions 資源的任務。
- `RUN_AT_START`: **true**: 在啟動時運行任務（如果啟用）。
- `SCHEDULE`: **@midnight**: Cron 語法，用於任務調度。

#### Cron - 清理已刪除的分支 (`cron.deleted_branches_cleanup`)

- `ENABLED`: **true**: 啟用已刪除分支清理。
- `RUN_AT_START`: **true**: 在啟動時運行任務（如果啟用）。
- `NOTICE_ON_SUCCESS`: **false**: 設定為 true 以記錄成功消息。
- `SCHEDULE`: **@midnight**: Cron 語法，用於安排已刪除分支清理。
- `OLDER_THAN`: **24h**: 刪除超過 OLDER_THAN 時間的已刪除分支。

#### Cron - 同步儲存庫許可證 (`cron.sync_repo_licenses`)

- `ENABLED`: **false**: 啟用儲存庫許可證同步。
- `RUN_AT_START`: **false**: 在啟動時運行任務（如果啟用）。
- `NOTICE_ON_SUCCESS`: **false**: 設定為 true 以記錄成功消息。
- `SCHEDULE`: **@annually*: Cron 語法，用於安排儲存庫許可證同步。

### 擴展的定時任務（預設未啟用）

#### Cron - 刪除所有儲存庫存檔 (`cron.delete_repo_archives`)

- `ENABLED`: **false**: 啟用服務。
- `RUN_AT_START`: **false**: 在啟動時運行任務（如果啟用）。
- `NOTICE_ON_SUCCESS`: **false**: 設定為 true 以打開成功通知。
- `SCHEDULE`: **@annually**: Cron 語法，用於刪除所有儲存庫存檔，例如 `@annually`。
...
- `RUN_AT_START`: **false**: 在啟動時運行任務（如果啟用）。
- `NOTICE_ON_SUCCESS`: **false**: 設定為 true 以打開成功通知。
- `SCHEDULE`: **@every 72h**: Cron 語法，用於安排 authorized_principals 更新，例如 `@every 1h`。
...
#### Cron - 檢查 Gitea 新版本 (`cron.update_checker`)
...
- `PROPORTION_TO_CHECK_PER_REPO`: **0.6**: 每個儲存庫至少檢查此比例的 LFSMetaObject。（這可能導致檢查所有過期的 LFSMetaObject。）

#<a id="actions-actions"></a>
## Actions 定時任務

#### Cron - 重建工單索引 (`cron.rebuild_issue_indexer`)
...
- `DIFF_RENAME_SIMILARITY_THRESHOLD`：**50%** 設定通過 `--find-renames=<threshold>` 傳遞給 git 命令的相似度閾值。預設值為 50%，與 git 相同。必須是 0% 到 100% 之間的整數百分比。
...
- `MERMAID_MAX_SOURCE_CHARACTERS`: **50000**: 設定 Mermaid 源的最大大小。(設為-1 代表禁止)

## Markup 外部渲染（`markup.external-render-name`）

Gitea 支援使用外部工具進行 Markup 渲染。以下範例將新增一個名為 `asciidoc` 的 Markup 渲染器。
...
- ENABLED：**false** 啟用 Markup 支援；設定為 **true** 以啟用此渲染器。
- FILE\_EXTENSIONS：**_empty_** 要由外部命令渲染的文件擴展名列表。多個擴展名需要用逗號分隔。
- RENDER\_COMMAND：用於渲染所有匹配的擴展名的外部命令。
- IS\_INPUT\_FILE：**false** 輸入不是標準輸入，而是一個在 `RENDER_COMMAND` 之後帶有文件參數的文件。
...
- RENDER_CONTENT_SANDBOX：**_empty_** 當 RENDER_CONTENT_MODE 為 `iframe` 時應用於 iframe 和 Content-Security-Policy 頭的沙箱設定。預設為一組安全的"allow-*"限制（空格分隔）。您也可以根據需要設定或使用"disabled"完全禁用沙箱。設定時請確保沒有安全風險：
    - 僅 PDF 內容：通常可以安全使用"disabled"，並且需要設定為"disabled"，因為 PDF 僅在無沙箱時渲染。
    - 包含 JS 的 HTML 內容：如果"RENDER_COMMAND"能保證不存在 XSS，則是安全的，否則需要微調"allow-*"限制。
- NEED_POST_PROCESS：**false** 是否對渲染後的 HTML 內容進行後處理，包括：解析相對鏈接和圖片源、識別 issue/commit 引用、轉義不可見字符、提及使用者、渲染永久鏈接程式碼塊、替換 emoji 短程式碼等。當 RENDER_CONTENT_MODE 為 `sanitized` 時預設為 true，否則為 false。
...
- `LIMIT_SIZE_TERRAFORM_STATE`：**-1**：Terraform state 上傳的最大大小（`-1` 表示無限制，格式為 `1000`、`1 MB`、`1 GiB`）。
...
- `STORAGE_TYPE`：**local**: lfs 的儲存類型，可以為空、`local`、`minio`、`azureblob` 或在 `[storage.xxx]` 中定義的其他名稱。
...
- `PATH`：**./data/lfs**: 儲存 LFS 文件的位置，僅在 `STORAGE_TYPE` 為 `local` 時可用。如果未設定，則回退到 `[server]` 部分中已棄用的 `LFS_CONTENT_PATH` 值。
  ...
  - `MINIO_BASE_PATH`：**attachments/**: 桶上的 Minio 基本路徑，僅在 `STORAGE_TYPE` 為 `minio` 時可用。
...
- `BATCH_SIZE`: **20**: 上游鏡像每次批量 API 請求的 LFS 指針數量。
- `BATCH_OPERATION_CONCURRENCY`: **8**: 批量操作中併發上傳/下載的數量。

## 儲存 (`storage`)

`attachments`、`lfs`、`avatars`、`repo-avatars`、`repo-archive`、`packages`、`actions_log`、`actions_artifact` 的預設儲存設定。推薦僅設定此部分並讓其他部分從此設定繼承，前提是所有儲存都位於同一父目錄或 Minio 桶下。

- `STORAGE_TYPE`：**local**: 儲存類型，`local` 表示本地磁盤，`minio` 表示 S3 相容對象儲存服務，`azureblob` 表示 Azure Blob 儲存服務。

<a id="storage_minio"></a>
### Minio 儲存設定（`storage_minio`）

- `SERVE_DIRECT`：**false**: 允許儲存驅動程式重定向到經過身份驗證的 URL 以直接提供文件。目前，僅支援通過簽名的 URL 提供 Minio/S3，本地不執行任何操作。
- `MINIO_ENDPOINT`：**localhost:9000**: Minio 連接終端點，僅在 `STORAGE_TYPE` 為 `minio` 時可用。
- `MINIO_ACCESS_KEY_ID`：Minio 的 accessKeyID，僅在 `STORAGE_TYPE` 為 `minio` 時可用。如果未提供且 `STORAGE_TYPE` 為 `minio`，將在已知的環境變量（MINIO_ACCESS_KEY_ID、AWS_ACCESS_KEY_ID）、憑據文件（~/.mc/config.json、~/.aws/credentials）以及 EC2 實例元資料中搜索憑據。
- `MINIO_SECRET_ACCESS_KEY`：Minio 的 secretAccessKey，僅在 `STORAGE_TYPE` 為 `minio` 時可用。
- `MINIO_IAM_ENDPOINT`：用於覆蓋 Minio 預設 IAM 終端點解析的首選 IAM 終端點，僅在 `STORAGE_TYPE` 為 `minio` 時可用。如果未提供且 `STORAGE_TYPE` 為 `minio`，將從已知的環境變量（AWS_CONTAINER_AUTHORIZATION_TOKEN、AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE、AWS_CONTAINER_CREDENTIALS_RELATIVE_URI、AWS_CONTAINER_CREDENTIALS_FULL_URI、AWS_WEB_IDENTITY_TOKEN_FILE、AWS_ROLE_ARN、AWS_ROLE_SESSION_NAME、AWS_REGION）中搜索和派生終端點，如果未另行提供則使用 DefaultIAMRoleEndpoint。
...
- `MINIO_BUCKET_LOOKUP_TYPE`：**auto**: Minio 的 bucket 查找方式預設為 `auto` 模式；可將其設定為 `dns`（虛擬託管樣式）或 `path`（路徑樣式），僅在 `STORAGE_TYPE` 為 `minio` 時可用。
...
### Azure Blob 儲存設定（`storage_azureblob`）

- `SERVE_DIRECT`：**false**: 允許儲存驅動程式重定向到經過身份驗證的 URL 以直接提供文件。目前，僅支援通過簽名的 URL 提供 Minio/S3，本地不執行任何操作。
- `AZURE_BLOB_ENDPOINT`：**_empty_**: Azure Blob 連接終端點，僅在 `STORAGE_TYPE` 為 `azureblob` 時可用。例如：https://accountname.blob.core.windows.net 或 http://127.0.0.1:10000/devstoreaccount1
...
### 覆蓋設定（`storage_override`）

覆蓋可以有 3 個層級。`[LFS]/[attachment]...` >> `[storage.xxx]` >> `[storage]` >> 預設值。
...
桶（bucket）、基本路徑或 `SERVE_DIRECT` 可以被特別指定或覆蓋，如果您想使用不同的設定，可以：
...
如果您想為 `lfs` 自訂一個不同的儲存（在上面已定義預設儲存的情況下）：
...
- `STORAGE_TYPE`：**local**: 儲存庫歸檔的儲存類型，可以為空、`local`、`minio`、`azureblob` 或在 `[storage.xxx]` 中定義的其他名稱。
  ...
  當 `STORAGE_TYPE = minio` 時，相關設定詳見 `Minio 儲存設定`，您可以如下覆蓋部分設定。

  - `MINIO_BASE_PATH`：**repo-archive/**: 桶上的 Minio 基本路徑，僅在 `STORAGE_TYPE` 為 `minio` 時可用。
...
- `WORKFLOW_DIRS`：**.gitea/workflows,.github/workflows**：以逗號分隔的工作流目錄列表，儲存庫中第一個存在的目錄將用於查找 Actions 工作流文件。
...
- `MAX_RERUN_ATTEMPTS`：**50**：單個工作流執行最多可以有的嘗試次數（初始執行 + 重新執行）。預設 50。可根據需要設定為任何正整數。
