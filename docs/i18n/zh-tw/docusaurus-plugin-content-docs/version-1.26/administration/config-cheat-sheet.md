...
slug: "config-cheat-sheet-zh-tw-v1.26"
sidebar_position: 30
...
- `STORAGE_TYPE`: **local**: 附件的儲存類型，可以為空、`local`、`minio`、`azureblob` 或在 `[storage.xxx]` 中定義的其他名稱。
  ...
  當 `STORAGE_TYPE = minio` 時，相關設定詳見 [Minio 儲存設定](#minio-儲存設定storage_minio)，您也可以如下定義設定來覆蓋派生的或預設的值。
...
- `STORAGE_TYPE`：**local**: 儲存庫歸檔的儲存類型，可以為空、`local`、`minio`、`azureblob` 或在 `[storage.xxx]` 中定義的其他名稱。
  ...
  當 `STORAGE_TYPE = minio` 時，相關設定詳見 [Minio 儲存設定](#minio-儲存設定storage_minio)，您可以如下覆蓋部分設定。
