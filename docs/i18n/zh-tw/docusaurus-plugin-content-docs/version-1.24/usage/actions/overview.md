...
Gitea Actions的Runner被稱為[act runner](https://gitea.com/gitea/act_runner)，它是一個獨立的程式，也是用Go語言編寫的。
它是基於[nektos/act](http://github.com/nektos/act)的一個[分支](https://gitea.com/gitea/act) 。

由於Runner是獨立部署的，可能存在潛在的安全問題。
為了避免這些問題，請遵循兩個簡單的規則：

- 不要為你的存放庫、組織或實例使用你不信任的Runner。
- 不要為你不信任的存放庫、組織或實例提供Runner。
