...
要告訴 Gitea 哪些文件需要展開，您必須在模板存放庫的 `.gitea` 目錄中包含一個 `template` 文件。
...
**注意：** 當從模板生成存放庫時，`.gitea` 目錄中的 `template` 文件將被刪除。
...
| REPO_LINK            | 生成的存放庫鏈接                | ✘      |
| TEMPLATE_LINK        | 模板存放庫鏈接                  | ✘      |
| REPO_HTTPS_URL       | 生成的存放庫的 HTTP(S) 克隆鏈接 | ✘      |
| TEMPLATE_HTTPS_URL   | 模板存放庫的 HTTP(S) 克隆鏈接   | ✘      |
| REPO_SSH_URL         | 生成的存放庫的 SSH 克隆鏈接     | ✘      |
| TEMPLATE_SSH_URL     | 模板存放庫的 SSH 克隆鏈接       | ✘      |
