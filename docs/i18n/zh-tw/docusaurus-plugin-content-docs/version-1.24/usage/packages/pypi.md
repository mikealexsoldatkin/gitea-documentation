...
要使用 PyPI 套件註冊表，您需要使用 [pip](https://pypi.org/project/pip/) 工具來消費和使用 [twine](https://pypi.org/project/twine/) 工具來發佈套件。
...
您可以使用 `--extra-index-url` 替代 `--index-url`，但這樣會使您容易受到依賴混淆攻擊，因為 `pip` 會先檢查官方 PyPi 存放庫中的套件，然後再檢查指定的自訂存放庫。請閱讀 `pip` 文件以獲取更多資訊。
