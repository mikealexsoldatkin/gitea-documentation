...
通過在 Gitea 的存放庫中調整 Git 的 `config` 文件，可以使用 `SIGNING_KEY=default` 為每個存放庫提供不同的簽名密鑰。然而，這顯然不是一個理想的使用者介面，因此可能會發生更改。
...
此選項確定在建立存放庫時，Gitea 是否應該對初始提交進行簽名。可能的取值有：
...
- `pubkey`：僅在使用者擁有公鑰時進行簽名
- `twofa`：僅在使用者使用 2FA 登入時進行簽名
- `basesigned`：僅在基礎存放庫中的父提交已簽名時進行簽名。
