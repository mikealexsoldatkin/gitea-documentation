...
slug: "config-cheat-sheet-zh-tw-v1.24"
sidebar_position: 30
...
- `FORCE_PRIVATE`: **false**: 強制使每個新存放庫變為私有。
- `DEFAULT_PRIVATE`: **last**: 建立新存放庫時預設為私有：`last`, `private`, `public`。
- `DEFAULT_PUSH_CREATE_PRIVATE`: **true**: 使用推送建立新存放庫時預設為私有。
- `MAX_CREATION_LIMIT`: **-1**: 每個使用者的全域存放庫建立上限,
  `-1` 代表無限制.
- `PREFERRED_LICENSES`: **Apache License 2.0,MIT License**: 要放置在列表頂部的指定許可證。
  名稱必須與 options/license 或 custom/options/license 中的文件名匹配。
- `DISABLE_HTTP_GIT`: **false**: 禁用 HTTP 協議與存放庫進行
  交互的能力。
- `USE_COMPAT_SSH_URI`: **false**: 當使用預設的 SSH 端口時，強制使用 ssh://克隆 URL，
  而不是 scp-style uri。
- `GO_GET_CLONE_URL_PROTOCOL`: **https**: 用於 "go get" 請求的值，返回存放庫的 URL 作為 https 或 ssh，
  預設為 https。
...
- `ENABLE_PUSH_CREATE_USER`: **false**: 允許使用者將本地儲存庫推送到 Gitea，並為使用者自動建立它們。
- `ENABLE_PUSH_CREATE_ORG`: **false**: 允許使用者將本地儲存庫推送到 Gitea，並為組織自動建立它們。
- `DISABLED_REPO_UNITS`: **_empty_**: 逗號分隔的全域禁用的存放庫單元列表。允許的值是：: \[repo.issues, repo.ext_issues, repo.pulls, repo.wiki, repo.ext_wiki, repo.projects, repo.packages, repo.actions\]
- `DEFAULT_REPO_UNITS`: **repo.code,repo.releases,repo.issues,repo.pulls,repo.wiki,repo.projects,repo.packages,repo.actions**: 逗號分隔的預設新存放庫單元列表。允許的值是：: \[repo.code, repo.releases, repo.issues, repo.pulls, repo.wiki, repo.projects, repo.packages, repo.actions\]. 注意：目前無法停用程式碼和發佈。如果您指定了預設的存放庫單元，您仍應將它們列出以保持未來的相容性。外部 wiki 和問題跟蹤器不能預設啟用，因為它需要額外的設定。禁用的存放庫單元將不會添加到新的存放庫中，無論它是否在預設列表中。
- `DEFAULT_FORK_REPO_UNITS`: **repo.code,repo.pulls**: 逗號分隔的預設分叉存放庫單元列表。允許的值和規則與`DEFAULT_REPO_UNITS`相同。
- `DEFAULT_MIRROR_REPO_UNITS`: **repo.code,repo.releases,repo.issues,repo.wiki,repo.projects,repo.packages**: 逗號分隔的預設鏡像存放庫單元列表。允許的值和規則與`DEFAULT_REPO_UNITS`相同。
- `DEFAULT_TEMPLATE_REPO_UNITS`: **repo.code,repo.releases,repo.issues,repo.pulls,repo.wiki,repo.projects,repo.packages**: 逗號分隔的預設模板存放庫單元列表。允許的值和規則與`DEFAULT_REPO_UNITS`相同。
- `PREFIX_ARCHIVE_FILES`: **true**: 通過將存檔文件放置在以存放庫命名的目錄中來添加前綴。
- `DISABLE_MIGRATIONS`: **false**: 禁用遷移功能。
- `DISABLE_STARS`: **false**: 禁用點贊功能。
- `DEFAULT_BRANCH`: **main**: 所有存放庫的預設分支名稱。
- `ALLOW_ADOPTION_OF_UNADOPTED_REPOSITORIES`: **false**: 允許非管理員使用者認領未被認領的存放庫。
- `ALLOW_DELETION_OF_UNADOPTED_REPOSITORIES`: **false**: 允許非管理員使用者刪除未被認領的存放庫。
- `DISABLE_DOWNLOAD_SOURCE_ARCHIVES`: **false**: 不允許從使用者介面下載源程式碼存檔文件。
- `ALLOW_FORK_WITHOUT_MAXIMUM_LIMIT`: **true**: 允許無限制得派生存放庫。

### 存放庫 - 編輯器 (`repository.editor`)
...
### 存放庫 - 合併請求 (`repository.pull-request`)
...
- `REOPEN_KEYWORDS`: **reopen**, **reopens**, **reopened**: 在拉取請求評論中用於自動重新打開相關問題的
  關鍵詞列表。
- `DEFAULT_MERGE_STYLE`: **merge**: 設定建立存放庫的預設合併方式，可選: `merge`, `rebase`, `rebase-merge`, `squash`, `fast-forward-only`
...
- `MAX_PINNED`: **3**: 每個存放庫的最大可固定工單數量。設定為 0 禁用固定工單。

### 存放庫 - 文件上傳 (`repository.upload`)

- `ENABLED`: **true**: 是否啟用存放庫文件上傳。
...
- `MERGES`: **pubkey, twofa, basesigned, commitssigned**: \[never, pubkey, twofa, approved, basesigned, commitssigned, always\]: 對合併操作進行簽名。
  ...
  - `basesigned`: 僅在基礎存放庫的父提交進行了簽名時才進行簽名。
  - `headsigned`: 僅在頭分支的頭提交進行了簽名時才進行簽名。
  - `commitssigned`: 僅在頭分支中的所有提交到合併點都進行了簽名時才進行簽名。

### 存放庫 - Local (`repository.local`)

- `LOCAL_COPY_PATH`: **tmp/local-repo**:臨時本地存放庫副本的路徑。預設為 tmp/local-repo（內容在 Gitea 重新啟動時被刪除）
...
- `EXPLORE_PAGING_NUM`: **20**: 探索頁面每頁顯示的存放庫數量。
...
- `SEARCH_REPO_DESCRIPTION`: **true**: 是否在探索頁面上的存放庫搜索中搜索描述。
- `ONLY_SHOW_RELEVANT_REPOS`: **false** 在沒有指定關鍵字並使用預設排序時，是否僅在探索頁面上顯示相關的存放庫。
  如果一個存放庫是分叉或者沒有元資料（沒有描述、圖標、主題），則被視為不相關的存放庫。

### 介面 - 管理員 (`ui.admin`)

- `USER_PAGING_NUM`: **50**: 單頁顯示的使用者數量。
- `REPO_PAGING_NUM`: **50**: 單頁顯示的存放庫數量。
- `NOTICE_PAGING_NUM`: **25**: 單頁顯示的通知數量。
- `ORG_PAGING_NUM`: **50**: 單頁顯示的組織數量。

### 介面 - 使用者 (`ui.user`)

- `REPO_PAGING_NUM`: **15**: 單頁顯示的存放庫數量。
...
- `STATIC_URL_PREFIX`: **_empty_**:
  ...
  靜態文件位於 Gitea 源程式碼存放庫的`public/`目錄中。
...
- `ENABLE_PPROF`: **false**: 應用程式分析（內存和 CPU）。對於 "web" 命令，它會在 `localhost:6060` 上監聽。對於 "serv" 命令，它會將資料轉儲到硬碟上的 `PPROF_DATA_PATH` 中，文件名為 `(cpuprofile|memprofile)_<username>_<temporary id>`。
...
- `REPO_INDEXER_ENABLED`：**false**: 啟用程式碼搜索（佔用大量硬碟空間，約為儲存庫大小的 6 倍）。
...
- `QUEUE_NAME`：**\_queue**: 預設的 redis 和硬碟隊列名稱的後綴。單獨的隊列將預設為 **`name`**`QUEUE_NAME`，但可以在特定的 `queue.name` 部分中進行覆蓋。
- `SET_NAME`：**\_unique**: 將添加到預設的 redis 和硬碟隊列 `set` 名稱中以用於唯一隊列的後綴。單獨的隊列將預設為 **`name`**`QUEUE_NAME`_`SET_NAME`_，但可以在特定的 `queue.name` 部分中進行覆蓋。
...
- `REPOSITORY_AVATAR_UPLOAD_PATH`: **data/repo-avatars**: 儲存存放庫頭像圖像文件的路徑。
- `REPOSITORY_AVATAR_FALLBACK`: **none**: Gitea 處理缺少存放庫頭像的方式
  - none = 不顯示任何頭像
  - random = 生成隨機頭像
  - image = 使用預設圖像（在 `REPOSITORY_AVATAR_FALLBACK_IMAGE` 中設定），如果設定為 image 並且未上傳任何圖像。
- `REPOSITORY_AVATAR_FALLBACK_IMAGE`: **/img/repo_default.png**: 作為預設存放庫頭像的圖像（如果將 `REPOSITORY_AVATAR_FALLBACK` 設定為 image 並且沒有上傳圖像）。
...
- `STORAGE_TYPE`: **local**: 附件的儲存類型，`local` 表示本地硬碟，`minio` 表示相容 S3 的對象儲存服務，如果未設定將使用預設值 `local` 或其他在 `[storage.xxx]` 中定義的名稱。
...
#### 定時任務 - 刪除舊的存放庫存檔 (`cron.archive_cleanup`)
...
#### 定時任務 - 更新鏡像存放庫 (`cron.update_mirrors`)
...
#### 定時任務 - 健康檢查所有存放庫 (`cron.repo_health_check`)

- `SCHEDULE`: **@midnight**: Cron 語法，用於安排存放庫健康檢查。
- `TIMEOUT`: **60s**: 用於健康檢查執行超時的時間持續語法。
- `ARGS`: **_empty_**: `git fsck` 命令的參數，例如 `--unreachable --tags`。在 http://git-scm.com/docs/git-fsck 上了解更多。

#### 定時任務 - 檢查所有存放庫統計 (`cron.check_repo_stats`)

- `RUN_AT_START`: **true**: 在啟動時運行存放庫統計檢查。
- `SCHEDULE`: **@midnight**: Cron 語法，用於安排存放庫統計檢查。
...
- `SCHEDULE`: **@every 72h**: Cron 語法，用於安排存放庫存檔清理，例如 `@every 1h`。
...
#### Cron - 重新同步所有存放庫的 pre-receive、update 和 post-receive 鉤子 (`cron.resync_all_hooks`)
...
#### Cron - 重新初始化所有缺失的 Git 存放庫，但記錄已存在 (`cron.reinit_missing_repos`)
...
#### Cron - 刪除所有缺少 Git 文件的存放庫 (`cron.delete_missing_repos`)
...
#### Cron - 刪除生成的存放庫頭像 (`cron.delete_generated_repository_avatars`)
...
#### Cron - 在存放庫中回收 LFS 指針 (`cron.gc_lfs`)
...
- `NUMBER_TO_CHECK_PER_REPO`: **100**: 每個存放庫要檢查的過期 LFSMetaObject 的最小數量。設定為 `0` 以始終檢查所有。
...
- `ALLOWED_DOMAINS`：**_empty_**: 允許遷移存放庫的域名允許列表，預設為空。這意味著允許外部網址。多個域名可以用逗號分隔。支援通配符：`github.com, *.github.com`。
- `BLOCKED_DOMAINS`：**_empty_**: 阻止遷移存放庫的域名阻止列表，預設為空。多個域名可以用逗號分隔。當 `ALLOWED_DOMAINS` 不為空時，此選項優先級較高，用於拒絕域名。支援通配符。
...
- `ENABLED`：**true**: 啟用鏡像功能。設定為 **false** 以禁用所有鏡像。預先存在的鏡像保持有效，但不會更新；可以轉換為常規存放庫。
...
- `STORAGE_TYPE`：**local**: lfs 的儲存類型，`local` 表示本地硬碟，`minio` 表示 S3 相容對象儲存服務，或者使用 `[storage.xxx]` 中定義的其他名稱。
...
預設的附件、lfs、頭像、存放庫頭像、存放庫歸檔、套件、操作日誌、artifacts 的儲存設定。推薦僅僅設定此 section 並讓其它的 section 從此設定項繼承。

- `STORAGE_TYPE`：**local**: 儲存類型，`local` 表示本地硬碟，`minio` 表示 S3，`azureblob` 表示 azure 對象儲存。
- `SERVE_DIRECT`：**false**: 允許儲存驅動程式重定向到經過身份驗證的 URL 以直接提供文件。目前，僅支援通過簽名的 URL 提供 Minio/S3，本地不執行任何操作。
- `MINIO_ENDPOINT`：**localhost:9000**: 連接的 Minio 終端點，僅在 `STORAGE_TYPE` 為 `minio` 時可用。
- `MINIO_ACCESS_KEY_ID`：Minio 的 accessKeyID，僅在 `STORAGE_TYPE` 為 `minio` 時可用。
- `MINIO_SECRET_ACCESS_KEY`：Minio 的 secretAccessKey，僅在 `STORAGE_TYPE` 為 `minio` 時可用。
...
- `STORAGE_TYPE`: **local**: 儲存類型，`local`表示本地硬碟，`minio`表示與 S3 相容的對象儲存服務，或者使用定義為`[storage.xxx]`的其他名稱。
...
- `STORAGE_TYPE`: **local**: 儲存類型，用於操作日誌，`local`表示本地硬碟，`minio`表示與 S3 相容的對象儲存服務，預設為`local`，或者使用定義為`[storage.xxx]`的其他名稱。
...
- `STORAGE_TYPE`: **local**: 用於操作日誌的儲存類型，`local`表示本地硬碟，`minio`表示與 S3 相容的對象儲存服務，預設為`local`，或者使用定義為`[storage.xxx]`的其他名稱。
...
- `LOG_COMPRESSION`: **zstd**: 日誌壓縮方式，`none`表示不壓縮，`zstd`表示 zstd 壓縮。
  ...
  如果 CPU 或內存不是瓶頸，建議在使用本地硬碟作為日誌儲存時總是使用壓縮。
