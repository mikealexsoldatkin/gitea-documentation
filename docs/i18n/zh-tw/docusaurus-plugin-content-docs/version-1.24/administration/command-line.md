...
- 選項：
  - `--scope {owner}[/{repo}]`，`-s {owner}[/{repo}]`：限制 Runner 的範圍，沒有範圍表示該 Runner 可用於所有存放庫，但你也可以將其限制為特定的存放庫或所有者。
...
要註冊特定存放庫的 Runner，例如 `username/test-repo`：
