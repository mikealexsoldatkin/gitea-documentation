...
Gitea 的首要目標是建立一個極易安裝，運行非常快速，安裝和使用體驗良好的自建 Git 服務。
...
- 程式碼評審：程式碼評審同時支援 Pull Request workflow 和 AGit workflow。評審人可以在線瀏覽程式碼，並提交評審意見或問題。提交者可以接收到評審意見，並在線回復或修改程式碼。程式碼評審可以幫助使用者和企業提高程式碼質量。
- CI/CD：Gitea Actions 支援 CI/CD 功能，該功能相容 GitHub Actions，使用者可以採用熟悉的 YAML 格式編寫 workflows，也可以重用大量的已有的 Actions 外掛。Actions 外掛支援從任意的 Git 網站中下載。
