...
受保護標籤允許控制誰有權建立或更新 Git 標籤。每個規則允許您匹配單個標籤名稱，或使用適當的模式來控制多個標籤。
...
1. 選擇允許的使用者和/或團隊。如果您將這些欄位留空，則沒有人可以建立或修改此標籤。
1. 選擇 **保存** 以保存設定。

## 模式受保護標籤

該模式使用 [glob](https://pkg.go.dev/github.com/gobwas/glob#Compile) 或正則表達式來匹配標籤名稱。對於正則表達式，您需要將模式括在斜槓中。
