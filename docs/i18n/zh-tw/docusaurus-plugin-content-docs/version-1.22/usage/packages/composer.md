...
為您的使用者或組織發佈 [Composer](https://getcomposer.org/) 套件。

## 要求

要使用 Composer 套件註冊表，您可以使用 [Composer](https://getcomposer.org/download/) 消費，並使用類似 `curl` 的 HTTP 上傳客戶端發佈套件。

## 發佈套件

要發佈 Composer 套件，請執行 HTTP `PUT` 操作，將套件內容放入請求體中。
...
| ----------------- | ----------------------------------------------------------- |
| `201 Created`     | 套件已發佈                                                |
| `400 Bad Request` | 套件名稱和/或版本無效，或具有相同名稱和版本的套件已存在 |

## 設定套件註冊表

要註冊套件註冊表，您需要將其添加到 Composer 的 `config.json` 文件中（通常可以在 `<user-home-dir>/.composer/config.json` 中找到）：
...
要使用憑據訪問套件註冊表，您必須在 `auth.json` 文件中指定它們，如下所示：
