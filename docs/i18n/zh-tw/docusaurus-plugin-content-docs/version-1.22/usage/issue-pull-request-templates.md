...
上面的範例表示使用者從列表中選擇一個工單模板時，列表會展示模板名稱 `Template Name` 和模板描述 `This template is for testing!`。 同時，標題會預先填充為 `[TEST]`，而正文將預先填充 `This is the template!`。 最後，Issue 還會被分配兩個標籤，`bug` 和 `help needed`，並且將問題指向 `main` 分支。
...
| ------------- | ----------------------------------------------------- | ---- | ------ | -------- | ------ |
| `label`       | 預期使用者輸入的簡短描述，以表單形式顯示。              | 必選 | 字符串 | -        | -      |
| `description` | 複選框集的描述，以表單形式顯示。 支援 Markdown 格式。 | 可選 | 字符串 | 空字符串 | -      |
| `options`     | 使用者可以選擇的複選框列表。 有關語法，請參閱下文。     | 必選 | 數組   | -        | -      |
