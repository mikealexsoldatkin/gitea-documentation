...
- 您在其他源中有一些舊專案，您不再主動使用，但出於歸檔目的不想刪除。在這種情況下，您可以建立一個推送鏡像，以便您的活躍的 Gitea 儲存庫可以將其更改推送到舊位置。

## 從遠程儲存庫拉取

對於現有的遠程儲存庫，您可以按照以下步驟設定拉取鏡像：

1. 在右上角的“建立...”菜單中選擇“遷移外部儲存庫”。
2. 選擇遠程儲存庫服務。
3. 輸入儲存庫的 URL。
...
現在，該儲存庫會定期從遠程儲存庫進行鏡像。您可以透過在儲存庫設定中選擇“立即同步”來強制進行同步。

:::warning
:exclamation::exclamation: **注意：**您只能為尚不存在於您的實例上的儲存庫設定拉取鏡像。一旦儲存庫建立成功，您就無法再將其轉換為拉取鏡像。:exclamation::exclamation:
:::

## 推送到遠程儲存庫

對於現有的儲存庫，您可以按照以下步驟設定推送鏡像：

1. 在儲存庫中，轉到**設定** > **儲存庫**，然後進入**鏡像設定**部分。
2. 輸入一個儲存庫的 URL。
3. 如果儲存庫需要身份驗證，請展開**授權**部分並填寫您的身份驗證資訊。請注意，所請求的**密碼**也可以是您的存取權杖。
4. 選擇**添加推送鏡像**以保存設定。

該儲存庫現在會定期鏡像到遠程儲存庫。您可以透過選擇**立即同步**來強制同步。如果出現錯誤，會顯示一條消息幫助您解決問題。

:::warning
:exclamation::exclamation: **注意：** 這將強制推送到遠程儲存庫。這將覆蓋遠程儲存庫中的任何更改！ :exclamation::exclamation:
:::
...
2. 在 GitHub 上建立一個同名的儲存庫。與 Gitea 不同，GitHub 不支援通過推送到遠程來建立儲存庫。如果您的現有遠程儲存庫與您的 Gitea 儲存庫具有相同的提交歷史，您也可以使用現有的遠程儲存庫。
...
當前，Gitea 不支援從 ssh 儲存庫進行鏡像。如果您想要鏡像一個 ssh 儲存庫，您需要將其轉換為 http 儲存庫。您可以使用以下命令將現有的 ssh 儲存庫轉換為 http 儲存庫：

1. 確保運行 gitea 的使用者有權限訪問您試圖從 shell 鏡像到的 git 儲存庫。
