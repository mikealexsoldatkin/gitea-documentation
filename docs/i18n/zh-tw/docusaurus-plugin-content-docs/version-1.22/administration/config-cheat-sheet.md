...
slug: "config-cheat-sheet-zh-tw-v1.22"
sidebar_position: 30
...
如果你發現 `%(X)s` 這樣的內容，請查看
[ini](https://github.com/go-ini/ini/#recursive-values) 這裡的說明。
標註了 :exclamation: 的設定項表明除非你真的理解這個設定項的意義，否則最好使用預設值。

在下面的預設值中,`$XYZ`代表環境變量`XYZ`的值(詳見:`environment-to-ini`)。 *`XxYyZz`*是指預設設定的一部分列出的值。這些在 app.ini 文件中不起作用，僅在此處列出作為文件說明。
...
- `ROOT`: **%(APP_DATA_PATH)s/gitea-repositories**: 存放 git 工程的根目錄，建議填絕對路徑。
  相對路徑將被解析為**_`AppWorkPath`_/%(ROOT)s**.
...
- `STATIC_URL_PREFIX`: **_empty_**:
  ...
  請求會變成 `%(ROOT_URL)s/static/assets/css/index.css` 或 `https://cdn.example.com/assets/css/index.css`
...
- `LOCAL_ROOT_URL`: **%(PROTOCOL)s://%(HTTP_ADDR)s:%(HTTP_PORT)s/**:
  ...
  `%(PROTOCOL)s://%(HTTP_ADDR)s:%(HTTP_PORT)s/`。如果監聽在 `0.0.0.0`，則預設值為
  `%(PROTOCOL)s://localhost:%(HTTP_PORT)s/`，
  否則預設值為 `%(PROTOCOL)s://%(HTTP_ADDR)s:%(HTTP_PORT)s/`。
- `LOCAL_USE_PROXY_PROTOCOL`: **%(USE_PROXY_PROTOCOL)s**: 在進行本地連接時傳遞 PROXY 協議頭。
...
- `BUILTIN_SSH_SERVER_USER`: **%(RUN_USER)s**: 用於內置 SSH 伺服器的使用者名稱。
- `SSH_USER`: **%(BUILTIN_SSH_SERVER_USER)s**: 在克隆 URL 中顯示的 SSH 使用者名稱。這僅適用於自行設定 SSH 伺服器的人；在大多數情況下，您希望將其留空並修改`BUILTIN_SSH_SERVER_USER`。
- `SSH_DOMAIN`: **%(DOMAIN)s**: 此伺服器的域名，用於顯示的克隆 URL。
...
- `LFS_CONTENT_PATH`: **%(APP_DATA_PATH)s/lfs**: 預設的 LFS 內容路徑（如果它在本地儲存中）。**已棄用**，請使用 `[lfs]` 中的設定。
...
- `REDIRECTOR_USE_PROXY_PROTOCOL`: **%(USE_PROXY_PROTOCOL)s**: 在連接到 https 重定向器時，需要 PROXY 協議頭。
...
- `DATADIR`：**queues/common**: 用於儲存 level 隊列的基本 DataDir。單獨的隊列的 `DATADIR` 可以在 `queue.name` 部分進行設定。相對路徑將根據 `%(APP_DATA_PATH)s` 變為絕對路徑。
...
- `ALLWAYS`: **false**: 設定為 true 以在 HTTP 和 HTTPS 內容上使用 camo，否則僅代理非 HTTPS URL。
